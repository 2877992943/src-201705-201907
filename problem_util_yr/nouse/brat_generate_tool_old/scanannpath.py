...
def read_file(text_file, annotation_file):
    #s_to_be_found = u'龟头老是流脓皮有点痒但是小便没有疼的感觉龟头眼旁边红肿为什么也没有感觉'
...
def read_path(path):
    ...
    for s in files:
        ...
        if os.path.isdir(son):
            temp_text, temp_annotation, temp_source, temp_no_text, temp_no_source = read_path(son)
            ...
            if son.endswith(u'.ann'):
                ...
                if os.path.exists(source_file):
                    ...
                    if len(temp_text)==0:
                        print 'this text not be worked',son
                        continue
                    text.extend(temp_text)
                    annotation.extend(temp_annotation)
                    source.extend(temp_source)
