...
from problem_util_yr.infer.get_chunk_symp_sign import limit_ceiling_floor,piece_join
...
def single_generator(inputfile_filename,nonEmpty_key,length_range=(0,100000)):#(0,100] (100,400],(400,]
    ...
    for line in reader.readlines():
        ...
        try:
            ...
            print 'fail in single generator',line
            continue
...
def cut_paragraph(text,position_from_cut_key,text_from_cut_key,dic_frame,maxlen_cut=100):
    """
    ...
    retll=[]
    ##
    further_comma_cut=False if maxlen_cut==-1 else True #如果切的长度-1 认为只用句号切不用逗号切
    sentll=limit_ceiling_floor(text,maxlen_cut,further_comma_cut=further_comma_cut)
    if maxlen_cut>0:
        sentll=piece_join(sentll,maxlen_cut,u'。') #将太短的连起来 chunk太短的预测不准   relation 用短的
    pos=0
    for sent in sentll:
        sent=sent.strip()
        ##过滤 空 字符串
        ...
        ###
        dic_frame[position_from_cut_key]=pos
        dic_frame[text_from_cut_key]=sent
        #dic_frame['type']='sent'
        pos+=1
...
def steps_paragraph_cut_uniqueSentence_sort_writeDisk(inputFile, inp_fname,outputFile,corpus,text_key=None,maxlen_cut=100):
    ...
    ## step 1 , cut  10G write into json
    writer_cut = open(os.path.join(outputFile, 'cut.json'), 'w')# 包含切句子和切之前的原文
    #### 切完句子的 KEY
    text_from_cut_key='cuttedSent_from_[%s]' % str(text_key)
    # 切完的位置的 KEY
    position_from_cut_key='cuttedSent_position_of_[%s]' % str(text_key)
    # unique sent id
    unique_sent_id='unique_cuttedSent_id_of_[%s]'%str(text_key)
    ###
    # text_key -> text_from_cut_key

    for d in dict_gene:
        d['corpus'] = corpus
        #d['type'] = 'raw'
        #d[position_from_cut] = 0
        text_before_cut = d[text_key]
        ##d['text']=text
        #del d[text_key]  # raw xbs paragraph
        ####
        dll = cut_paragraph(text_before_cut,
                            position_from_cut_key,text_from_cut_key,
                            d,maxlen_cut=maxlen_cut)
        for d in dll:  # sent
            if len(d[text_from_cut_key]) < 3: continue  # 去掉太短的切的句子
            if len(d[text_from_cut_key]) >500: continue  # 去掉太长的切的句子
            writer_cut.write(json.dumps(d, ensure_ascii=False) + '\n')
    writer_cut.close()

    ####### step 2 ,unique
    writer_cut1 = open(os.path.join(outputFile, 'cut_with_uniqueSentenceID.json'), 'w')#只包含切完的句子，不包含切之前的句子refer to cut.json
    writer_uniqueSentId=open(os.path.join(outputFile, 'uniqueSentenceID.json'), 'w')
    print ''
    dict_gene = single_generator(os.path.join(outputFile, 'cut.json'), text_from_cut_key)#
    uniqueSentence2id_dict={}
    for d in dict_gene:
        sent=d[text_from_cut_key]
        if sent in uniqueSentence2id_dict:
            sent_id=uniqueSentence2id_dict[sent]
        else: # 新句子
            uniqueSentence2id_dict[sent]=len(uniqueSentence2id_dict)
            sent_id=uniqueSentence2id_dict[sent]
            ##
            #writer_uniqueSentId.write(json.dumps({'id':sent_id,text_from_cut:sent,'unique_sent_id':sent_id},ensure_ascii=False)+'\n')
            writer_uniqueSentId.write(json.dumps({text_from_cut_key: sent,
                                                  unique_sent_id: sent_id}, ensure_ascii=False) + '\n')
        ### 原始数据中 加key 找到 unique sentence
        d[unique_sent_id]=sent_id
        ##
        writer_cut1.write(json.dumps(d, ensure_ascii=False) + '\n')
    ###
    writer_uniqueSentId.close()
    writer_cut1.close()
    ###
    ...
    # sort & groupbylength
    dict_gene = single_generator(os.path.join(outputFile, 'uniqueSentenceID.json'), text_from_cut_key)



    for key, group in groupby(sorted(dict_gene, key=lambda s: len(str(s.get(text_from_cut_key)).decode('utf-8')), reverse=True),
                              key=lambda s: len(str(s.get(text_from_cut_key)).decode('utf-8'))):
...
def steps_xbsRelation_afterInferProcess_writeInDisk(inputsList,retll,dic_cache,enlabel,writer_tmp):
    ...
    for ii in range(len(retll)):
        ...
        try:
            ...
            d_chunk_result['relation_infer_result'] = result
...
def main(data_dir,
    ...
    if start_file_ind==0:
        ### 在outputFile 建立新的 result/ sort/ unique_filtered/
        if os.path.exists(os.path.join(outputFile,'result'))==True:
            shutil.rmtree(os.path.join(outputFile,'result'))
        os.makedirs(os.path.join(outputFile,'result'))
        if os.path.exists(os.path.join(outputFile,'sort'))==True:
            shutil.rmtree(os.path.join(outputFile,'sort'))
        os.makedirs(os.path.join(outputFile,'sort'))
        if os.path.exists(os.path.join(outputFile,'unique_filtered'))==True:
            shutil.rmtree(os.path.join(outputFile,'unique_filtered'))
        os.makedirs(os.path.join(outputFile,'unique_filtered'))
    ...
    if start_file_ind == 0:
         #一律切句子
        print '切句子...'
        steps_paragraph_cut_uniqueSentence_sort_writeDisk(inputFile, inp_fname, outputFile, corpus, text_key,maxlen_cut)
    #
    #study_distribution()
    #### 切完句子的 KEY
    text_from_cut_key = 'cuttedSent_from_[%s]' % str(text_key)
    ...
    for fname in fll[start_file_ind:]:
        ...
        # data generator
        data_gene_single = single_generator(fname, text_from_cut_key)
        ...
        hparams_key_value=None#'worker_gpu=2'
        ...
        for dic_cache in data_gene_cache:  # dic_cache [d,d] d={'vid':xxx,'xbs':xxx  # such as 10000 obs
            ...
            input_string_ll = [dic[text_from_cut_key].decode('utf-8') for dic in dic_cache]
            if len(input_string_ll) == 0: continue
            if extralen==0:
                inputsList, retll = decoder.infer_batch_seq2seq(input_string_ll, True)
            else:
                inputsList, retll = decoder.infer_batch_seq2seq(input_string_ll,False,multiply_length=extralen)  # list
