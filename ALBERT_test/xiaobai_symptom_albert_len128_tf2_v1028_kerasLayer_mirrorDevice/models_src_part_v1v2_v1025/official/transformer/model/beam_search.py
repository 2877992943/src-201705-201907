...
class SequenceBeamSearch(object):
  ...
  def _create_initial_state(self, initial_ids, initial_cache):
    ...
    finished_flags = tf.zeros([self.batch_size, self.beam_size], tf.bool,name='finish_flag')# yr
  ...
  def _get_new_finished_state(self, state, new_seq, new_log_probs):
    ...
    if not self.padded_decode:
      finished_seq = tf.concat([
          finished_seq,
          tf.zeros([self.batch_size, self.beam_size, 1], tf.int32,name='new_fini_state')
      ],
                               axis=2)# yr
