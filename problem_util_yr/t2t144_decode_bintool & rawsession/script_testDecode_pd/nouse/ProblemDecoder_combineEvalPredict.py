...
class ProblemDecoder_predict(object):
    ...
    def _init_env(self):
        ...
        ####### problem type :输出分类 还是序列
        self.problem_type = self._hparams.problems[0].target_modality[0]
    ...
    def infer_singleSample(self, input_string, decode_length_x):
        ...
        ### beam size>1,
            ...
            return results,score2prob(predictions_score)
    ...
    def infer_batch(self,inputsList):
        ...
        for b in range(num_decode_batches):
            ...
            #     num,beam_result,_ = pred_results.shape
            #     #split_shape = 1 if split_shape == 0 else split_shape
            #     predictions_score = predictions_score[:,:beam_result]
            #     np_predictions_list = np.split(pred_results, num, axis=0)  # [6,10]->[[1,10],[],[]...]
            #     for pred in np_predictions_list:
            #         results = [targets_vocab.decode(beam.flatten()) for beam in pred]
            #         print '\n'.join(results)
            #     print ''
...
class ProblemDecoder_eval(object):
    ...
    def _init_env(self):
        ...
        self._features['targets']=targets
    ...
    def eval_singleSample(self, input_string, decode_length_x):
        #encoder = self._hparams.problem_instances[0].feature_encoders(self._data_dir)["inputs"]
        #decoder = self._hparams.problem_instances[0].feature_encoders(self._data_dir)["targets"]
        input_key='targets' if 'inputs' not in self._hparams.problems[0].vocabulary else 'inputs'
        inputs_vocab = self._hparams.problems[0].vocabulary[input_key]
...
if __name__ == "__main__":
    ...
    inps = ['女 (18,30] 面部红斑 关节痛', '女 (18,30] 脱发', '女 (18,30] 面部红斑 关节痛'][:2]
    # inps = ['女 (18,30] 面部红斑 关节痛 脱发 关节痛']

    result, score = pd.infer_singleSample(inps[0], 10)  # greedy_result 10 beam_result 14 ,input(partial target)=4

    print ''
    # result, score = pd.infer_singleSample(inps[1], 10)
    # inputsList,retll=pd.infer_batch(inps)
