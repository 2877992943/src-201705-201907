...
LATENT_SIZE = 256#128  # Hard-code latent layer sizes for demos.
VOCABSZ_SYMPTOM=FLAGS.node_vocabsz
LATENT_SIZE_SMALL = 256



def get_mlp_model_steps(name='steps', hidsz=LATENT_SIZE):
  mlp_mod = snt.nets.MLP([hidsz] * NUM_LAYERS, activate_final=True, name=name)
  return mlp_mod


def map2sym(name='tosym', hidsz=VOCABSZ_SYMPTOM):
  mlp_mod = snt.nets.MLP([hidsz] * NUM_LAYERS, activate_final=True, name=name)
  return mlp_mod
...
def make_mlp_model_small():
  ...
  """
  return snt.Sequential([
      snt.nets.MLP([LATENT_SIZE_SMALL] * NUM_LAYERS, activate_final=True),
      snt.LayerNorm()
  ])
...
class MLPGraphNetwork(snt.AbstractModule):
  ...
  def __init__(self, name="MLPGraphNetwork"):
    ...
    with self._enter_variable_scope():
      self._network = modules.GraphNetwork(edge_model_fn=make_mlp_model_small,
                                           node_model_fn=make_mlp_model,
                                           global_model_fn=make_mlp_model_small)
...
class EncodeProcessDecode(snt.AbstractModule):
  ...
  def _build(self, input_op, num_processing_steps):
    ...
    #
    latent = self._encoder(input_op) # graph_out=GN(graph_in)  latent  [? 256]
    latent0 = latent #[?node_n,hid16] node[? 16] edge [? 16]
    output_ops = []
    for _ in range(num_processing_steps):# how many cores GN block
      core_input = utils_tf.concat([latent0, latent], axis=1)# node 256 -> 512 ,edge 25->50
