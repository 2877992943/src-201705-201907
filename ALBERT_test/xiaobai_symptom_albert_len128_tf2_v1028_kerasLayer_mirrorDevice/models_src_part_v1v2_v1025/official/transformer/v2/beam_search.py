...
class SequenceBeamSearchV2(v1.SequenceBeamSearch):
  ...
  def search(self, initial_ids, initial_cache):
    ...
    ###
    state_l=[state]
    for i in range(self.max_decode_length.numpy()):
    #for i in range(10):
        state_l=self._search_step(state_l[0])
        not_end=self._continue_search(state_l[0])
        not_end=not_end.numpy()
        if not_end==False:break
        #print ('')

    ###
    finished_state=state_l
