...
class ProblemDecoder(object):
    ...
    def _init_env(self):
        ...
        self.targets_ph_2 = tf.placeholder(tf.int32, shape=(None, None, 1, 1), name='targets')
        ...
        with self._sess.as_default():
            ...
            if os.path.exists(os.path.join(self._model_dir, "checkpoint")):
                saver.restore(self._sess, tf.train.latest_checkpoint(self._model_dir))
            ...
            for _ in tqdm(range(global_step, 1000 + 1)):
                ...
                loss, train_op = sess.run([self.ret2, model.train_op], feed_dict={
    ...
    def infer_singleSample(self, input_string, y,y1,decode_length_x_mutiply,position_max=10):
        ...
        targets1=targets_vocab.encode(y)+[1];
        targets2 = targets_vocab.encode(y1) + [1];
        ...
        with self._sess.as_default():
            ...
            target_pretend=np.expand_dims(np.expand_dims(np.array([targets1,targets1]),axis=2),axis=3)
            target_pretend2 = np.expand_dims(np.expand_dims(np.array([targets2, targets2]), axis=2), axis=3)

            feed = {self.inputs_ph: inputs_,
                    self.input_extra_length_ph: decode_length_extra,
                    self.targets_ph: target_pretend,
                    self.targets_ph_2:target_pretend2}
...
if __name__=='__main__':
    ...
    for d in gene:
        ...
        annotationll_1.append(d['y1'])


    #####
    for ii in range(len(textll))[:2]:#2:3 not nanloss   | 1:2 nanloss
        x =textll[ii]
        y=annotationll[ii]
        y1=annotationll_1[ii]
        #if len(x.split(' '))<10 or len(y.split(' '))<10:continue
        decoder.infer_singleSample(x,y,y1,0)
