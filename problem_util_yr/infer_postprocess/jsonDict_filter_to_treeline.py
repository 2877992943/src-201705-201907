...
def jsonDict_filter(rootll,text,en2cn_dict): #4 layer  同时 enlabelId -> cnlabelId
    # filter 1)empty None 2)invalidWord not in text, and  enlabel->cnlabel
    def whether_continue(root='not filled',word='not filled',labelId='not filled'):
        if root!='not filled': # fill root
            ...
            if text!=None and word not in text:return True #预测生成的词  不在原文中 跳过
        return False

    def transfer_fn(word):
        if en2cn_dict==None:
            return word
        else:
            return en2cn_dict[word]
    ...
    for root in rootll:
        ...
        if flag:continue
        if att==[]:
            all_ll.append({'word':word,'labelId':transfer_fn(labelId),'att':[]})
        elif att!=[]:
            att_=[]
            for root1 in att:
                ...
                if flag:continue
                if att1==[]:
                    att_.append({'word':word1,'labelId':transfer_fn(labelId1),'att':[]})
                elif att1!=[]:
                    att1_=[]
                    for root2 in att1:
                        ...
                        if flag:continue
                        if att2==[]:
                            att1_.append({'word': word2, 'labelId': transfer_fn(labelId2), 'att': []})
                        elif att2!=[]:
                            att2_=[]
                            for root3 in att2:
                                ...
                                if flag:continue
                                if att3==[]:
                                    att2_.append({'word': word3, 'labelId': transfer_fn(labelId3), 'att': []})

                            att1_.append({'word': word2, 'labelId': transfer_fn(labelId2), 'att': att2_})


                    att_.append({'word': word1, 'labelId': transfer_fn(labelId1), 'att': att1_})



            all_ll.append({'word': word, 'labelId': transfer_fn(labelId), 'att': att_})
...
def jsonDict_to_treeline(rootll): #4 level
...
def get_all_route_from_rootlist_4layerDeep(rootll,en2cn_dict):
    all_route_path_ll=[]
    ...
    for root in rootll:
        ...
        if word.strip()=='':continue
        if att==[]:
            all_route_path_ll.append([word,en2cn_dict[labelId],
                                      '','',
                                      '','',
                                      '',''])
        elif att!=[]:
                            ...
                            for root3 in att2:
                                ...
                                if att3==[]:
                                    all_route_path_ll.append([word, en2cn_dict[labelId],
                                                              word1, en2cn_dict[labelId1],
                                                              word2, en2cn_dict[labelId2],
                                                              word3,en2cn_dict[labelId3]])

    return all_route_path_ll
