...
class ProblemDecoder_predict(object):
    ...
    def infer_singleSample(self, input_string, inputLengh_required_equalto_outputLength,
        ...
        ## 分类问题 如果是 transformer score model 直接返回   分类问题 返回所有VOCAB的概率
            ...
            rawarr=np.squeeze(predictions_dict['predictions'])

            top5prob = [s for s in np.sort(rawarr)[::-1][:self._classification_problem_topN]]
            top5=np.argsort(rawarr)[::-1][:self._classification_problem_topN]
            for id in top5:
