...
class ProblemDecoder_predict(object):
    ...
    def _init_env(self):
        ...
        if self._customer_problem_type=='languageModel_pp':
            ...
            del self._features['input_space_id']
            del self._features['target_space_id']
        if self._model_name in ['slice_net']:
            del self._features['problem_choice']
            del self._features['input_space_id']
            del self._features['target_space_id']
        ...
        if self._model_name.lower().find('score')!=-1 and self._customer_problem_type=='classification':
        ...
        if self._score!=None: #[batch,beam] [batch,]
            self._predictions_dict['scores_yr']=tf.exp(common_layers.log_prob_from_logits(self._score))
    ...
    def infer_singleSample(self, input_string, inputLengh_required_equalto_outputLength,
        ...
        with self._sess.as_default():
            ...
            ##如果是分类问题 需要每个vocab的概率
                ...
                if self._model_name.lower().find('scorer')!=-1:
                    ...
                    raise ValueError('for classification problem,model should use transformer_score_t2t162!!! and you did not')
        ...
        if self._customer_problem_type=='languageModel_pp':
            ...
            pp = []
            for stepid in range(len(inputs_id)):
                vocab_id_this_step = inputs_id[stepid]
                prob = arr[stepid, vocab_id_this_step]
                pp.append(prob)
            #print pp
            #return np.mean([np.exp(-p) for p in pp])
            return np.mean(pp)
        ## 分类问题 如果是 transformer score model 直接返回   分类问题 返回所有VOCAB的概率
        elif self._customer_problem_type=='classification' and self._model_name.lower().find('scorer') != -1:
            top5prob,top5str=[],[]
            #arr=np.squeeze(predictions_dict['scores_yr'])# will repeat logsumexp operation
            arr = np.exp(np.squeeze(predictions_dict['scores']))
        ...
        predictions_score=np.squeeze(predictions_dict.get('scores_yr'),axis=0)
    ...
    def infer_batch_seq2seq(self,inputsList,inputLengh_required_equalto_outputLength,
        ...
        for b in range(num_decode_batches):
            ...
            with self._sess.as_default():
                ...
                if self._customer_problem_type=='classification':
                    if self._model_name.lower().find('scorer') != -1:
                        ...
                        raise ValueError(
                            'for classification problem,model should use transformer_score_t2t162!!! and you did not')
            ...
            if self.problem_type == 'class_label' and self._model_name.lower().find('scorer') != -1:
                ...
                prediction_scores=predictions_dict['scores']#[batch,step=1,vocab=212]
            ...
            ## 其他问题 seq2seq2, language model
                ...
                predictions_score=predictions_dict.get('scores_yr')
                ###greedy infer,no score , no return beam
