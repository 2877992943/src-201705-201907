...
def remove_none(dic):
    if type(dic)!=dict:return dic
    kll=[k for k,v in dic.items() if v==None]
    for k in kll:
        del dic[k]
    return dic
...
class ProblemDecoder(object):
    def __init__(self, problem, model_dir, model_name, hparams_set, usr_dir,
                ...
                force_decode_length=True,custom_problem='classification'):
        #
        ...
        #self.customer_problem=custom_problem# 例如seq2seq  classification
    ...
    def infer_singleSample(self, input_string, decode_length_x_mutiply,position_max=10):
        ...
        with self._sess.as_default():
            ...
            try:
                ########
                # run sesion 序列
                self.outputs_scores=remove_none(self.outputs_scores)
                ...
                # ############
                self.logits=remove_none(self.logits)
        ...
        logits_mat=logits_mat.flatten()#->[vocabsz,]

        ##### get output stri
        sample=outputs_scores['outputs'].flatten()#class output[1,]
        stri=targets_vocab.decode(sample.flatten())
        name_logit={}
        for n in range(len(logits_mat)):
            name=targets_vocab.decode([n])
            if len(name)>0:
                name_logit[name[0]]=logits_mat[n]

        ll = sorted(name_logit.iteritems(), key=lambda s: s[1], reverse=True)[:5]
        return ll
