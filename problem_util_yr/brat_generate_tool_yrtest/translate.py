...
if __name__=='__main__':
    ...
    allLabels=[l.strip().decode('utf-8') for l in allLabels.split('\n')]
    f='relation_train_corpus_1213.txt'
    sentencell,treelinell=read_data_from_treebank(f)
    f='relation_train_corpus_1214.txt'
    s1,t1=read_data_from_treebank(f)
    f='relation_train_corpus1220.txt'
    s2,t2=read_data_from_treebank(f)

    pd.to_pickle([sentencell+s1+s2,treelinell+t1+t2],'sentence_treeline.pkl')
    num=len(sentencell+s1+s2)
    print num
