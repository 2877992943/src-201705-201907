...
class AttentionVisualizer(object):
  ...
  def decode_list_targets(self, integers):
    ...
    #return self.encoders['targets'].decode(integers)
    return self.encoders['targets'].output_idsList2strList(integers)
  def get_vis_data_from_string(self, sess, input_string,extra_length_multiply):
    ...
    if self.beam_size==1:
        ...
        print ('') #[1,beam,steps]
        outputs,scores=out.get('outputs'),out.get('scores')#[1,beam,step]
    ...
    if self.beam_size==1:
        ...
        return {0:[output_string, input_list, output_list, att_mats]}
    elif self.beam_size>1:
        beam_rst={}
        for beam_i,out in beam_out.items():
            ...
            att_mats = sess.run(self.att_mats, feed);print np.std(att_mats[2])
            # att_mats = sess.run(self.att_mats, [feed])
            ids = []
            squeezedIds = np.squeeze(out)
            for x in np.nditer(squeezedIds):
...
def get_att_mats(translate_model,model_name):
  ...
  elif 'multi_model' in model_name:
    decoder_layers = translate_model.hparams.num_hidden_layers
  ...
  elif model_name=='multi_model_yr':
      key_mat=dict()
      for key,mat in translate_model.attention_weights.items():
        if 'encdec' in key and 'logits' not in key:
            encdec_atts.append({key:mat})
        if 'self_attention' in key and 'decoder' in key and 'logits' not in key:
            dec_atts.append({key:mat})
