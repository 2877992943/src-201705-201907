...
# from tensor2tensor.tpu import tpu_trainer_lib as trainer_util
...
class ProblemDecoder(object):
  ...
  def _init_env(self):
    ...
    self._hparams = tpu_trainer_lib.create_hparams(hparams_set=self._hparams_set,data_dir=self._data_dir,problem_name=self._problem)
    ...
    decode_hparams = decoding.decode_hparams(self._hparams_dc)
    model_fn = t2t_model.T2TModel.make_estimator_model_fn(
      self._model_name,
      self._hparams,
      decode_hparams=decode_hparams)
    estimator_spec = model_fn(features,'',mode)
  ...
  def __init__(self,problem,model_dir,model_name,hparams_set,usr_dir,
    ...
    # decode_hp.add_hparam("beam_size", beam_size)
    # decode_hp.add_hparam("alpha", alpha)
    # decode_hp.add_hparam("extra_length", extra_length)
    # decode_hp.add_hparam("return_beams", return_beams)
    # decode_hp.add_hparam("use_last_position_only", use_last_position_only)
    ...
    self._hparams_dc='beam_size=%d,alpha=%f,extra_length=%d,return_beams=False'%(beam_size,alpha,extra_length)
    self._init_env()
  ...
  def infer_batch(self,input_string_batch): #[xxxx,xxx]
    ...
    self._lock.acquire()
    with self._sess.as_default():
        feed = {self._inputs_ph: batch_input_id}
        start = time.time()
  ...
  def infer_singleSample(self, input_string):
    encoder = self._hparams.problem_instances[0].feature_encoders(self._data_dir)["inputs"]
    decoder = self._hparams.problem_instances[0].feature_encoders(self._data_dir)["targets"]
    ...
    logging.getLogger("nlp").info("Inferring result is [%s] for [%s]", results, input_string)
    return results
