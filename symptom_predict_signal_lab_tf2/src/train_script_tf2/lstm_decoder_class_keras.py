...
class lstmDecoder(layers.Layer):
    def __init__(self,vocab_size,latent_dim,train_flag):
        ...
        self.hparams = lstm_yr.lstm_seq2seq()
        self.hparams.hidden_size=latent_dim
        ...
        self.embeddings_y = tf.Variable(
            tf.random.uniform([vocab_size, latent_dim], -1.0, 1.0))


        ####### stack(lstmcell)
        # self.decoder_l = lstm_yr.lstm(
        ...
        ###
        self.decoder_l=layers.LSTM(self.hparams.hidden_size,
                                   #return_state=True,
        ...
        self.dense_l=layers.Dense(vocab_size)




    def call_train(self,
    #def create_model_encode_decode(self,
                                   #inputs,
                                   y_id,
                                   final_encoder_state): # inp[batch step 1 hid]  yid[batch step 1 1]
        ...
        if 2>1:
            ...
            y = tf.nn.embedding_lookup(params=self.embeddings_y, ids=y_id)
            ...
            # LSTM encoder.
            #inputs = tf.reverse_sequence(input=inputs, seq_lengths=inputs_length, seq_axis=1)
            #_, final_encoder_state = \
            #self.enc_=lstm_yr(#inputs, inputs_length,
            #                   hparams, train_flag,"encoder")  # finale_encode_state must be lstmStateTuple
            ...
            stat=tf.squeeze(final_encoder_state,axis=[1,2])#->[n,step,dim]
            inp_y_emb=common_layers.flatten4d3d(shifted_targets)
            decoder_outputs=self.decoder_l(inputs=inp_y_emb,
                                           #initial_state=final_encoder_state
                                           initial_state=[stat,stat]
                                           )

            # decode output [batch step hid]
            decoder_outputs = self.dense_l(decoder_outputs)
            # ->[batch step vocabsz]
    ...
    def sample(self,features):
        y_id=features['targets']
        encode_state=features['inputs']
        logits = self.call_train(y_id,
                           encode_state)#[batch step 1 vocabsz]
        samples = tf.argmax(input=logits, axis=-1)
        # sample [3,?,1] logit [3 ? 1 vocabsz]
        samples = self.tensor3dto4d(samples)
        # ->sample [batch step 1 1] logit[batch step 1 1 vocabsz]
    ...
    def _slow_greedy_infer_yr(self,features, decode_length=0):
        ...
        def infer_step(recent_output, recent_logits, unused_loss):  # output [batch step 1 1]
            ...
            padded = tf.pad(tensor=recent_output, paddings=[[0, 0], [0, 1], [0, 0], [0, 0]])  # [3 0 1 1]
    ...
    def call(self,inputs):
        if self.train_flag==True:#train
            y,inputs=inputs
            return self.call_train(y, inputs)

        else:# infer
            return self._slow_greedy_infer_yr(inputs, 10)
...
if __name__=='__main__':
    ...
    if train_flag:
        ...
        rst=instan.call_train(y,inputs)
