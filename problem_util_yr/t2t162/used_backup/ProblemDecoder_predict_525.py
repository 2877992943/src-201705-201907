...
class ProblemDecoder_predict(object):
    ...
    def _init_env(self):
        ...
        if self._model_name.lower().find('score')!=-1:
            ...
            #self._target_pretend=np.zeros((2,1,1,1))
        ...
        if self._score!=None: #[batch,beam] [batch,]
            self._score = tf.exp(common_layers.log_prob_from_logits(self._score))
            #self._score = tf.exp(self._score)
        self._predictions_dict['scores_yr']=self._score
    ...
    def infer_singleSample(self, input_string, inputLengh_required_equalto_outputLength,
                           multiply_length=2,eos_required=False):
        """
        ...
        temporally not allowed to be True in chunk problem
        """
        ...
        if inputLengh_required_equalto_outputLength==True: #chunk problem, input_length=output_length
            ...
            if multiply_length>2 or type(multiply_length)!=int:
                multiply_length = 2
        ...
        with self._sess.as_default():
            ...
            self._target_pretend=np.zeros((1, 1, 1, 1))
            if self.problem_type == 'class_label':
