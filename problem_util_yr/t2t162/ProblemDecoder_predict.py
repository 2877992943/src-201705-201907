...
class ProblemDecoder_predict(object):
    def __init__(self, problem, model_dir, model_name, hparams_set, usr_dir,
                 ...
                 force_decode_length=True,problem_type='seq2seq',extra_length=0):
        ...
        # 没什么用暂时，此处只是CREAT DECODE HPARAM用 ,在 infer方法中单独设置起作用 在transformer
        #transformer 的时候 可以初始化时候传  也可以传入样本时候single_infer(input,decode_extra_length)
        # 但是universal的时候 只能在初始化类的时候传这个
    ...
    def _init_env(self):
        ...
        x.set_shape([None, None]) # ? -> (?batch,?steps)
        ...
        if self._model_name=='transformer' and self._customer_problem_type=='classification':
            ...
            del self._features['input_space_id']
            del self._features['target_space_id']
        if self._model_name=='transformer' and self._customer_problem_type=='seq2seq':
            del self._features['problem_choice']
            del self._features['input_space_id']
            del self._features['target_space_id']
        if 'multi_model' in self._model_name:
            del self._features['problem_choice']
            del self._features['input_space_id']
            del self._features['target_space_id']
        if self._customer_problem_type=='languageModel_seq':
            del self._features['problem_choice']
            del self._features['input_space_id']
            del self._features['target_space_id']
    ...
    def infer_singleSample(self, input_string, inputLengh_required_equalto_outputLength,
        ...
        self._lock.acquire()
        with self._sess.as_default():
            tf.logging.info('decode extra length multiply %s,len of input %s', multiply_length, len(inputs_id))
        ...
        ####
        if self._customer_problem_type=='languageModel_pp':
            arr = np.squeeze(predictions_dict['predictions'],
                             axis=[0, 1, 3])  # [1,1,step,1,vocab] -> [step,vocab] logit=logProb
        ...
        if self._customer_problem_type=='seq2seq':
            if predictions_dict.get('scores').shape.__len__()>=1:
                predictions_score=predictions_dict.get('scores')#[1,]
        else:
        ...
        if self._beam_size==1:
            ...
            if len(predictions_score.shape)==4:
                seq_score=np.squeeze(np.max(predictions_score,axis=-1))
                return [results], seq_score
    ...
    def infer_batch_seq2seq(self,inputsList,inputLengh_required_equalto_outputLength,
        ...
        #batch_size只是控制1个BATCH最多几个，例如10,零散的例如3个，自己一个batch,1个batch几个 是可变same as decode_length
        sorted_inputs.reverse()
        batch_size=self._batch_size
        max_input_size=1000
        #eos_required=False
        ...
        for b in range(num_decode_batches):
            ...
            ## 其他问题 seq2seq2, language model
                ...
                predictions_score=predictions_dict.get('scores')#[batchsz,]
                ###greedy infer
                if self._beam_size == 1: ##   序列2序列  非语言模型问题
                    ...
                    score_this_batch=list(predictions_score)
                    finalResultsList+=zip(results,score_this_batch)
