...
from plain_utf8_encoder_xbsNew import PlainUTF8Encoder_input,PlainUTF8Encoder_output
...
def model_fn(inputs, targets1,targets2, mode, params):
  ...
  with tf.variable_scope("model"):
    ...
    logits,logits2 = model(inputs, targets1,targets2)
    #logits2 = model(inputs, targets2) ### wrong this way,encode same w, but decode cannot use same w
    ...
    logits.set_shape(targets1.shape.as_list() + logits.shape.as_list()[2:])
    logits2.set_shape(targets2.shape.as_list() + logits2.shape.as_list()[2:])

    # Calculate model loss.
    # xentropy contains the cross entropy loss of every nonpadding token in the
    # targets.
    xentropy, weights = metrics.padded_cross_entropy_loss(
        logits, targets1, params["label_smoothing"], params["vocab_size_out"])#logits[? ? 5] target[? ?] xentro[? ?]
    xentropy2, weights2 = metrics.padded_cross_entropy_loss(
        logits2, targets2, params["label_smoothing"], params["vocab_size_out"])
    loss = tf.reduce_sum(xentropy) / tf.reduce_sum(weights)
    loss2 = tf.reduce_sum(xentropy2) / tf.reduce_sum(weights2)
    loss+=loss2
...
def gene_data():
    ...
    f = '../data/corpus/train.json'
    from problem_util_yr.loadDict.read_json_tool import read_json
    gene = read_json(f)
    datall = []
    for d in gene:
        x, y, y1 = d['x'], d['y'], d['y1']
        x = input_encoder.encode(x) + [1]
        y = [3]+output_encoder.encode(y) + [1]# 3 start of y
        y1 = [4]+output_encoder.encode(y1) + [1] # 4 start of y1
        datall.append({'inputs': np.array([x]), 'targets': np.array([y]), 'targets2': np.array([y1])})

    return datall


    #pdd.to_pickle(datall, 'datall.pkl')
...
if __name__ == "__main__":
  ...
  #FLAGS.vocab_file='../data/dict/'
  FLAGS.param_set='tiny'


  ### encoders
  cpath=FLAGS.data_dir
  inDict = "/dict/unigram_5k.txt"  # char
  outDict = "/dict/labelDict.txt"  #
  input_encoder = PlainUTF8Encoder_input(cpath + inDict, cpath + outDict)
  output_encoder = PlainUTF8Encoder_output(cpath + inDict, cpath + outDict)

  ### data
  datall=gene_data()
  #datall=pdd.read_pickle('datall.pkl')
