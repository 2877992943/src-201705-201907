...
def steps_xbsParagraph_cut_sort_writeInDisk(inputFile, inp_fname,outputFile,corpus,text_key):
    ...
    for d in dict_gene:
        ...
        for d in dll:  # sent
            if len(d['text']) < 3: continue  # 去掉太短的切的句子
...
def steps_zhusuParagraph_sort_writeInDisk(inputFile, inp_fname,outputFile,corpus,text_key):
    # corpus='zhusu'
    dict_gene = single_generator(os.path.join(inputFile, inp_fname), text_key)
    # # cut  10G write into json
    writer_cut = open(os.path.join(outputFile, 'cut.json'), 'w')
...
def main(paramlist,start_file_ind=0):
    ...
    corpus, cut_sentenct_flag, enlabel, enlabel_need, OOM_LIMIT,extralen,text_key=paramlist
    ...
    for fname in fll[start_file_ind:]: #一个文件 是全部一个长度的数据
        ...
        writer_tmp = open(os.path.join(outputFile, 'result','allchunk_tmp_result_batchsize%d_length%d.json' % (batch_size_this_lengthRange, length_this_file)), 'w')
        ...
        for dic_cache in data_gene_cache:  # dic_cache [d,d] d={'vid':xxx,'xbs':xxx  # such as 10000 obs
            ...
            inputsList, retll = decoder.infer_batch_seq2seq(input_string_ll,inputLengh_required_equalto_outputLength=True)  # list
