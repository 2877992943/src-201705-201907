...
class ProblemDecoder(object):
    def __init__(self, problem, model_dir, model_name, hparams_set, usr_dir,
        ...
        #
        self.get_logits=False# when infer ,run session to get logits too slow
        ...
        if self.predict_or_eval == 'and':
            ...
            with tf.variable_scope(tf.get_variable_scope(), reuse=True):
                self.outputs_scores = translate_model.infer(features=self._features,
                                                            ...
                                                            alpha=self._alpha)
            ####
            if self.outputs_scores['scores']==None:
                self.outputs_scores['scores']=tf.constant(1)
    ...
    def infer_singleSample(self, input_string, decode_length_x_mutiply,position_max=10):
        ...
        with self._sess.as_default():
            ...
            try:
                ...
                if self._beam_size==1:
                    ...
                    target_pretend=sample.reshape([1,-1,1,1]) ## sample output -> logits
                    if self.get_logits==True:
                ...
                #  多 个 beam
                    ...
                    outputs =outputs_scores['outputs']##[1,beam,step][batch,beam,step]
        ...
        if self.get_logits==False:
            rsts=[]
            if self._beam_size==1:
                sample = outputs_scores['outputs'].flatten()
                rsts.append(targets_vocab.decode(sample))

            elif self._beam_size>1:
                samples=outputs_scores['outputs'][0]#[batch,beam,step]->[beam,step]
                for ii in range(samples.shape[0]):
                    sample=samples[ii,:]
                    rst = targets_vocab.decode(sample)
                    rsts.append(rst)
            return rsts
